...
export default (boardSize, mineNum) => {
    ...
    for(let x = 0; x < boardSize; x++){
        for(let y = 0; y < boardSize; y++){
            if(board[x][y].value !== '💣'){
                if(x-1 >= 0 && board[x-1][y].value === '💣') {
                    board[x][y].value += 1;
                }
                if(y-1 >= 0 && board[x][y-1].value === '💣') {
                    board[x][y].value += 1;
                }
                if(x+1 < boardSize && board[x+1][y].value === '💣') {
                    board[x][y].value += 1;
                }
                if(y+1 < boardSize && board[x][y+1].value === '💣') {
                    board[x][y].value += 1;
                }
                if(y-1 >= 0 && x-1>= 0 && board[x-1][y-1].value === '💣') {
                    board[x][y].value += 1;
                }
                if(y+1 < boardSize && x+1 < boardSize && board[x+1][y+1].value === '💣') {
                    board[x][y].value += 1;
                }
                if(y-1 >= 0 && x+1 < boardSize && board[x+1][y-1].value === '💣') {
                    board[x][y].value += 1;
                }
                if(y+1 < boardSize && x-1>= 0 && board[x-1][y+1].value === '💣') {
                    board[x][y].value += 1;
                }
            }
        }
